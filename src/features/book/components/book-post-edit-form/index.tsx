...
export const BookPostEditForm = ({ post }: BookPostEditFormProps) => {
  const { mutate, isPending } = useUpdateBookPostMutation();
  ...
  const onSubmit = (data: EditFormValues) => {
    ...
    const payload: UpdateBookPostParams = {
      title: data.title,
    ...
    <Card className="w-full">
      ...
      <CardContent>
        <div className="flex items-center p-4 mb-6 border rounded-lg bg-gray-50">
          ...
          />
          <div className="ml-4">
            <p className="font-semibold text-gray-800">{post.book.title}</p>
            <p className="text-sm text-gray-500">{post.book.author} 저</p>
          </div>
        </div>
        <Form {...form}>
